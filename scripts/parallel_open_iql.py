...
if __name__ == "__main__":
    ...
    for i in range(switch_interval):
        ...
        env_copy.id = i + 1
        tmp_records_folder = f"{records_folder}_{i+1}"
        tmp_plots_folder = f"{plots_folder}_{i+1}"
        env_copy.plotter_params["records_folder"] = tmp_records_folder
        env_copy.plotter_params["plots_folder"] = tmp_plots_folder
        env_copy.recorder = Recorder(env_copy.plotter_params)
        shutil.copytree(records_folder, tmp_records_folder, dirs_exist_ok=True)
        shutil.copytree(plots_folder, tmp_plots_folder, dirs_exist_ok=True)
        env_copy.simulator.sumo_id = f"{env_copy.simulator.sumo_id}_{i+1}"
        env_copy.simulation_params["records_folder"] = tmp_records_folder
        env_copy.simulation_params["plots_folder"] = tmp_plots_folder
        env_copy.simulator = SumoSimulator(env_copy.simulation_params, None, env_copy.seed, True, False)
    ...
    episode = 0
    while episode < training_eps + dynamic_episodes:
        env.reset()
